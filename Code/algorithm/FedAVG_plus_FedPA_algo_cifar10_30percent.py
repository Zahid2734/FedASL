...
def FedAVG_plus_FedPA_algo_cifar10(comms_round, client_taken, data ,x_test,y_test , client_percent):
    ...
    for comm_round in range(comms_round):
        ...
        for i in range(len(randomlist)):
            ...
            pre_train_data={}
            pre_train_data[client_names[randomlist[i]]]= batch_data(data[client_names[randomlist[i]]][0:150])
            prefilter_time = time.time()
            local_score= local_model.evaluate(pre_train_data[client_names[randomlist[i]]], verbose=0)
            pretrain_time= time.time()-prefilter_time
            print("time taken for pretrain and accuracy",pretrain_time, local_score[1])
            ...
            post_accurcy= local_model.evaluate(pre_train_data[client_names[randomlist[i]]], verbose=0)
            accuracy_list.append(post_accurcy[1])
            print("time_for only training and accuracy ", time.time()-train_time, post_accurcy[1])
            if comm_round>9:

                if abs(post_accurcy[1] - local_score[1]) < 2*standard_dev and (median - 1.5*standard_dev) < local_score[1]:
                    print('passed')
                    data_points = 500
                ...
                if (post_accurcy[1] - local_score[1]) >= 2*standard_dev:
                ...
                total_data.append(data_points)
                scaled_local_weight_list.append(local_model.get_weights())
                passed_client.append(post_accurcy[1])
            # clear session to free memory after each communication round
            local_complete= time.time()-local_time
            print('time taken in client_side and percetage of overhead added by pretrain', local_complete, pretrain_time/local_complete)
            K.clear_session()
