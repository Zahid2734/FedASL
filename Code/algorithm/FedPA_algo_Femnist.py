...
from Code.utils.model import SimpleMLP3, SimpleMLP2,test_model
...
def FedPA_algo_Femnist(comms_round, client_taken, clients_batched,x_test,y_test , client_percent, alpha, beta, previous_average):
    alpha= alpha
    beta= beta
    lr = 0.01
    ...
    for comm_round in range(comms_round):
        ...
        for i in range(len(randomlist)):
            ...
            if comm_round>50:
                if abs(history.history['accuracy'][-1]-local_score[1])< .20 and median -.02<local_score[1]:
                    print('passed')
