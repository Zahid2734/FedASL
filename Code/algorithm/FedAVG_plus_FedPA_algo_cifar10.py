...
def FedAVG_plus_FedPA_algo_cifar10(comms_round, client_taken, clients_batched,x_test,y_test , client_percent):
    alpha= 1
    beta= .2
    lr = 0.01
    ...
    for comm_round in range(comms_round):
        ...
        for i in range(len(randomlist)):
            ...
            print("time_for only training ", time.time()-train_time)
            if comm_round>9:

                if abs(history.history['accuracy'][-1] - local_score[1]) < 0.15 and (median - .05) < local_score[1]:
                    print('passed')
