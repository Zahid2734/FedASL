...
from Code.utils.math_function import weight_scalling_factor,fed_avg_weight,scale_model_weights,sum_scaled_weights,weight_std_dev_median, trimmed_mean_algo_mnist,weight_alpha_beta_median
...
def TM_plus_FedPA_algo_mnist(comms_round, client_taken, data ,x_test,y_test , client_percent,percent_trim):
    alpha= 1
    beta= .2
    lr = 0.01
    ...
    for comm_round in range(comms_round):
        ...
        for i in range(len(randomlist)):
            ...
            # print("time_for only training and accuracy ", time.time()-train_time, post_accurcy)
            if comm_round > 9:
        ...
        weighted_value = trimmed_mean_algo_mnist(scaled_local_weight_list, percent_trim)
